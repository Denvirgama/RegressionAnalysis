...
plot_scatter <- ggplot(mydata, aes(x = PTLike, y = LPConsumer)) +
              geom_point() + 
...
fit_full_2 <- lm(formula = log(LPConsumer) ~ PTLike + Paid + TotalInterac + 
...
# removing variable due to t-value & F-value: category1 
fit_full_3 <- lm(formula = log(LPConsumer) ~ PTLike + Paid + TotalInterac + 
                   typeP + typeS + typeV, data = mydata)
...
fit_full_4 <- lm(formula = log(LPConsumer) ~ PTLike + TotalInterac + 
                   typeP + typeS + typeV, data = mydata)
...
fit_inter_Paid_full_1 <- lm(log(LPConsumer) ~ Paid*(PTLike  + typeP + typeS + TotalInterac +
...
# analysis of variance
anova(fit_inter_Paid_2)


# remove variable due to F-value: category1
fit_inter_Paid_3 <- lm(formula = log(LPConsumer) ~ Paid + PTLike + typeP + typeS + 
                TotalInterac + typeV + Paid:TotalInterac, data = mydata)
...
# full interaction model
fit_full_interaction_1 <- lm(log(LPConsumer) ~ (PTLike + Paid + typeP + typeS + TotalInterac +
...
fit_full_interaction_2 <- lm(formula = log(LPConsumer) ~ PTLike + Paid + typeP + typeS + 
                          TotalInterac + typeV + category1 + PTLike:typeP + 
                          PTLike:TotalInterac + Paid:typeP + Paid:TotalInterac +
                          typeP:TotalInterac + typeS:TotalInterac + TotalInterac:typeV +
                          TotalInterac:category1, data = mydata)
...
fit_full_interaction_3 <- lm(formula = log(LPConsumer) ~ PTLike + Paid + typeP + typeS + 
                          TotalInterac + typeV + category1 + PTLike:typeP + 
                          PTLike:TotalInterac + Paid:TotalInterac +
                          typeP:TotalInterac + typeS:TotalInterac + TotalInterac:typeV +
                          TotalInterac:category1, data = mydata)
...
# remove interaction variable due to t-value: PTLike:TotalInterac
fit_full_interaction_4 <- lm(formula = log(LPConsumer) ~ PTLike + Paid + typeP + typeS + 
                          TotalInterac + typeV + category1 + PTLike:typeP + 
                          Paid:TotalInterac + typeP:TotalInterac + typeS:TotalInterac +
...
# Model: 1 : fit_full_4
fit_m1_trn <- lm(formula = log(LPConsumer) ~ PTLike + TotalInterac + 
                   typeP + typeS + typeV, data = train.mydata)
...
fit_int_m1_trn <- lm(formula = log(LPConsumer) ~ Paid + PTLike + typeP + typeS + 
                TotalInterac + typeV + Paid:TotalInterac, data = train.mydata)
...
fit_int_m2_trn <- lm(formula = log(LPConsumer) ~ PTLike + Paid + typeP + typeS + 
                          TotalInterac + typeV + category1 + PTLike:typeP + 
                          Paid:TotalInterac + typeP:TotalInterac + typeS:TotalInterac +
                          TotalInterac:typeV + TotalInterac:category1, data = train.mydata)
...
# m1 = matrix(c(rmse_m1, rmse_m1_2, rmse_m3, mae_m1, mae_m1_2, mae_m3, mape_m1, mape_m1_2, 
#               mape_m3), nrow = 3)
# 
# 
# colnames(m1) <- c('RMSE', 'MAE', 'MAPE')
# rownames(m1) <- c('fit_m1_trn', 'fit_inter_Paid_3', 'fit_full_interaction_4')
# 
# # display the matrics
# m1
...
#             fit_m1_trn 0.8154020 0.4909182 8.991299 0.10741474
#       fit_inter_Paid_3 0.7364274 0.4747493 8.758684 0.04807864
# fit_full_interaction_4 0.6711453 0.4451866 8.382681 0.03597338
